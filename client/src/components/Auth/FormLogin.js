...
import { useState, useEffect } from 'react'; // добавил
import * as actions from '../../store/actions/user';
import store from '../../store'; // добавил

export default function FormLogin() {
  const allState = store.getState(); // добавил
  ...
  useEffect(() => { // добавил
    if (allState.user.data.auth && allState.user.data.auth.username === 'Данные введены неверно!') {
      console.log(allState.user.data.auth.username);
